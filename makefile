...
build_cuda_code: build_rapids_env build_python build_cython
	/usr/local/cuda-11.3/bin/nvcc --shared -o libgpu_dim_reduction.so \
		cython/cuda_wrappers/gpu_dim_reduction.cpp \
		cython/cuda_kernels/gpu_kernels.cu \
		cython/utils/gpu_utils.cu \
		-Xcompiler -fPIC
	/usr/local/cuda-11.3/bin/nvcc --shared -o libgpu_graph_weights.so \
...
run_gidr_dun_gpu_example: build_gpu_code
