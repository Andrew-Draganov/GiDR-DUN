...
def optimize_sampling(
    ...
    """
    # - If we optimize an edge, then the next epoch we optimize it is
    ...
    for i in numba.prange(epochs_per_sample.shape[0]):
        if epoch_of_next_sample[i] <= i_epoch:
            ...
            # pick random vertex from knn for calculating attractive force
            ...
            # - This accounts for the (1 - w(x, y)) in the repulsive grad coefficient
            ...
            # - UMAP performs multiple repulsive actions for each attractive one!
            ...
            for p in range(n_neg_samples):
                ...
                for d in range(dim):
                    # tSNE doesn't do gradient clipping
...
def optimize_uniformly(
    ...
    for i in numba.prange(epochs_per_sample.shape[0]):
        ...
        # Picks random vertex from ENTIRE graph and calculates repulsive force
        # If we are summing the effects of the forces and multiplying them
